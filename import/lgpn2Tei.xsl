...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <TEI xmlns="http://www.tei-c.org/ns/1.0">
            <teiHeader>
                ...
                <encodingDesc>
                    <classDecl>
                        <!-- flat taxonomy with <catDesc> entries for every language; links to appropriate ontologies realised through corresp attribute  -->
                        <taxonomy xml:id="semanticFields">
                            ...
                            <xsl:for-each select="distinct-values(tokenize($interps, ', '))">
                                <category xml:id="{.}"><catDesc xml:lang="en"><xsl:value-of select="."></xsl:value-of></catDesc></category>
                            </xsl:for-each>
    ...
    <xsl:template match="tei:m">
        ...
        <xsl:copy>
            <xsl:if test="string($ana)"><xsl:attribute name="ana" select="normalize-space($ana)"/></xsl:if>
            <xsl:apply-templates select="@*|text()"/>
        </xsl:copy>
    </xsl:template>
