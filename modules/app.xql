...
declare variable $app:i18n-files := "/db/apps/lgpn-ling/data/i18n";
...
declare
%templates:wrap
%templates:default("lang", "fr")
function app:search-dialect-value($node as node(), $model as map(*), $lang as xs:string?) {
    for $dialect in  doc("/db/apps/lgpn-ling-data/data/taxonomies/dialects.xml")//TEI:category
        return
            element option {
                attribute value { $dialect/@xml:id/string() },
                $dialect/TEI:catDesc[@ana="full"][@xml:lang=$lang]/text()
            }
};
